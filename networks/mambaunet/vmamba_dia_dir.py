...
class CrossMerge(torch.autograd.Function):
    ...
    def forward(ctx, ys: torch.Tensor):
        ...
        y1 = ys[:, 0].view(B, 1, D, -1)
        y2 = ys[:, 1].view(B, 1, D, -1)
        y3 = ys[:, 2].flip(dims=[-1]).view(B, 1, D, -1)
        y4 = ys[:, 3].flip(dims=[-1]).view(B, 1, D, -1)
        y1 = diagonal_scatter(y1, (B, D, H, W))
        y2 = antidiagonal_scatter(y2, (B, D, H, W))
        y3 = diagonal_scatter(y3, (B, D, H, W))
        y4 = antidiagonal_scatter(y4, (B, D, H, W))
        y_res = (y1+y3)+(y2+y4)
        return y_res.view(B, D, -1), [y1, y2, y3, y4]

    @staticmethod
    # def backward(ctx, x: torch.Tensor):
    def backward(ctx, x: torch.Tensor, list):
        # B, D, L = x.shape
        # out: (b, k, d, l)
        H, W = ctx.shape
        B, C, L = x.shape
...
def cross_selective_scan(
    ...
    y, [y1, y2, y3, y4] = CrossMerge.apply(ys)
    y = y.view(B, D, -1)            # [24, 192, 3136]
    y1 = y1.view(B, H, W, -1)       # [24, 56, 56, 192]
    y2 = y2.view(B, H, W, -1)
    y3 = y3.view(B, H, W, -1)
    y4 = y4.view(B, H, W, -1)
    ...
    if out_norm_shape in ["v1"]:  # (B, C, H, W)
        ...
        y = out_norm(y).view(B, H, W, -1)       # [24, 56, 56, 192]

    return (y.to(x.dtype) if to_dtype else y), [y1, y2, y3, y4]
...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        y, [y1, y2, y3, y4] = self.forward_core(x)
...
class VSSBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        x, [y1, y2, y3, y4] = self.self_attention(self.ln_1(input))
...
class VSSM(nn.Module):
    ...
    def forward_features(self, x):
        ...
        return x, x_downsample, y_

    def out_features(self, x, shape):
        B, H, W, C = x.shape
        x = x.permute(0, 3, 1, 2)  # B,C,H,W
        conv = nn.Conv2d(in_channels=C, out_channels=self.num_classes, kernel_size=1, bias=False).cuda()
        x = conv(x)
        x = F.interpolate(x, shape)
        return x

    # Dencoder and Skip connection
    def forward_up_features(self, x, x_downsample, shape):
        y_ = []
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                x, feat = layer_up(x)
                for i in range(4):
                    feat[i] = self.out_features(feat[i], shape)
                y_.append(feat)
    ...
    def forward(self, x):
        shape = x.shape[2:]
        x, x_downsample, y_encoder = self.forward_features(x)
        x, y_decoder = self.forward_up_features(x, x_downsample, shape)
        x = self.up_x4(x)
        return x, y_encoder[3]
