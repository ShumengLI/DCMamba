...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        y = out_y[:, 0] + inv_y[:, 0] + wh_y + invwh_y          # [24, 192, 3136]
        y = torch.transpose(y, dim0=1, dim1=2).contiguous().view(B, H, W, -1)       # [24, 56, 56, 192]
        y1 = torch.transpose(out_y[:, 0], dim0=1, dim1=2).contiguous().view(B, H, W, -1)      # [24, 56, 56, 192]
        y2 = torch.transpose(inv_y[:, 0], dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        y3 = torch.transpose(wh_y, dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        y4 = torch.transpose(invwh_y, dim0=1, dim1=2).contiguous().view(B, H, W, -1)
...
class VSSM(nn.Module):
    ...
    def forward_up_features(self, x, x_downsample, shape):
        ...
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = layer_up(x)         # patch expand
            else:
                x = torch.cat([x,x_downsample[3-inx]],-1)
                x = self.concat_back_dim[inx](x)
                x, feat = layer_up(x)
                for i in range(4):
                    feat[i] = self.out_features(feat[i], shape)         # [24, 9, 224, 224]
                y_.append(feat)
