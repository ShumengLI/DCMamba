...
from .vmamba_dia_dir import VSSM

logger = logging.getLogger(__name__)

class MambaUnet_dia(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        super(MambaUnet_dia, self).__init__()
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.config = config

        # print("patch_size: ", config.MODEL.VSSM.PATCH_SIZE)
        self.mamba_unet = VSSM(
                                patch_size=config.MODEL.VSSM.PATCH_SIZE,
                                in_chans=config.MODEL.VSSM.IN_CHANS,
                                num_classes=self.num_classes,
                                embed_dim=config.MODEL.VSSM.EMBED_DIM,
                                depths=config.MODEL.VSSM.DEPTHS,            # [2,2,2,2]
